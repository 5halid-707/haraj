...
async function seedIfEmpty() {
  ...
  const listings = [
    // CARS (3)
    { title: 'تويوتا كامري 2022 فل كامل', description: 'تويوتا كامري 2022 فل اوبشن كامل ماشية 45 الف كم بحالة الوكالة صيانة كاملة', price: 85000, currency: 'ريال', city: 'الرياض', district: 'النسيم', categoryId: allCats.find(c=>c.slug==='cars')!.id, userId: adminUser.id, images: JSON.stringify([img('1503376780353-7e6692767b70')]), status: 'active', isFeatured: true, views: 245, year: 2022, kilometers: 45000, condition: 'مستعمل', phone: '0575015019', whatsapp: '966575015019' },
    { title: 'هيونداي سوناتا 2021', description: 'هيونداي سوناتا 2021 فل اوبشن ماشية 60 الف كم بحالة ممتازة صيانة دورية', price: 55000, currency: 'ريال', city: 'جدة', district: 'الروضة', categoryId: allCats.find(c=>c.slug==='cars')!.id, userId: adminUser.id, images: JSON.stringify([img('1583121274602-3e2820c69888')]), status: 'active', views: 180, year: 2021, kilometers: 60000, condition: 'مستعمل', phone: '0557654321', whatsapp: '966557654321' },
    { title: 'نيسان باترول 2020', description: 'نيسان باترول 2020 فل كامل ماشية 80 الف كم بحالة الوكالة فحص شامل', price: 180000, currency: 'ريال', city: 'الدمام', district: 'الشاطئ', categoryId: allCats.find(c=>c.slug==='cars')!.id, userId: adminUser.id, images: JSON.stringify([img('1533473359331-0135ef1b58bad')]), status: 'active', isFeatured: true, views: 320, year: 2020, kilometers: 80000, condition: 'مستعمل', phone: '0561112233', whatsapp: '966561112233' },
    { title: 'مرسيدس C200 2023', description: 'مرسيدس C200 2023 AMG فل كامل ماشية 15 الف كم بحالة الوكالة', price: 220000, currency: 'ريال', city: 'الرياض', district: 'العليا', categoryId: allCats.find(c=>c.slug==='cars')!.id, userId: adminUser.id, images: JSON.stringify([img('1555215695-3004980ad54e')]), status: 'active', views: 410, year: 2023, kilometers: 15000, condition: 'مستعمل', phone: '0575015019', whatsapp: '966575015019' },

    // REAL ESTATE (3)
    { title: 'فيلا 400م في حي الياسمين', description: 'فيلا دورين 400م مربع 5 غرف 4 حمامات بموقف سيارة حديقة', price: 1200000, currency: 'ريال', city: 'الرياض', district: 'الياسمين', categoryId: allCats.find(c=>c.slug==='real-estate')!.id, userId: adminUser.id, images: JSON.stringify([img('1568605114967-8130f3a36994')]), status: 'active', isFeatured: true, views: 410, phone: '0575015019', whatsapp: '966575015019' },
    { title: 'شقة 180م في حي الروضة', description: 'شقة 180م مربع 3 غرف 2 حمامات مطبخ راكب تكييف', price: 450000, currency: 'ريال', city: 'جدة', district: 'الروضة', categoryId: allCats.find(c=>c.slug==='real-estate')!.id, userId: adminUser.id, images: JSON.stringify([img('1493809842364-78817add7ffb')]), status: 'active', views: 195, phone: '0557654321', whatsapp: '966557654321' },
    ...
    // ELECTRONICS (3)
    { title: 'آيفون 15 برو ماكس 256GB', description: 'آيفون 15 برو ماكس 256 جيجا لون تيتانيوم طبيعي بحالة الوكالة ضمان', price: 5499, currency: 'ريال', city: 'الرياض', categoryId: allCats.find(c=>c.slug==='electronics')!.id, userId: adminUser.id, images: JSON.stringify([img('1592286927505-1def25115558')]), status: 'active', views: 89, condition: 'جديد', phone: '0575015019', whatsapp: '966575015019' },
    { title: 'ماك بوك برو 14 إنش M3', description: 'ماك بوك برو 14 إنش M3 برو 512 جيجا 16 رام بحالة ممتازة', price: 8499, currency: 'ريال', city: 'جدة', categoryId: allCats.find(c=>c.slug==='electronics')!.id, userId: adminUser.id, images: JSON.stringify([img('1611186871348-b1ce696e52c9')]), status: 'active', views: 145, condition: 'مستعمل', phone: '0557654321', whatsapp: '966557654321' },
    { title: 'سامسونج تلفاز 55 إنش QLED', description: 'سامسونج تلفاز 55 إنش QLED 4K ذكي بحالة ممتازة', price: 3199, currency: 'ريال', city: 'الدمام', categoryId: allCats.find(c=>c.slug==='electronics')!.id, userId: adminUser.id, images: JSON.stringify([img('1593784991095-a205069470b6')]), status: 'active', views: 67, condition: 'مستعمل', phone: '0561112233', whatsapp: '966561112233' },
    ...
    { title: 'كنبة جلد 3 مقاعد إيطالية', description: 'كنبة جلد طبيعي 3 مقاعد تصميم إيطالي فخم', price: 1800, currency: 'ريال', city: 'جدة', district: 'الروضة', categoryId: allCats.find(c=>c.slug==='furniture')!.id, userId: adminUser.id, images: JSON.stringify([img('1567538096630-e0c55bd6374c')]), status: 'active', views: 56, condition: 'مستعمل', phone: '0557654321', whatsapp: '966557654321' },
    ...
    { title: 'مزرعة نخيل 3000م مع بئر', description: 'مزرعة نخيل 3000م بها 50 نخلة وبئر ارتوازي', price: 450000, currency: 'ريال', city: 'القصيم', district: 'بريدة', categoryId: allCats.find(c=>c.slug==='farm')!.id, userId: adminUser.id, images: JSON.stringify([img('1591857177580-dc82b9ac4e1e')]), status: 'active', views: 89, phone: '0561112233', whatsapp: '966561112233' },
    ...
    { title: 'طيور زينة مستوردة', description: 'طيور زينة مستوردة بأنواع مختلفة ببغاء كاسكو كناري', price: 800, currency: 'ريال', city: 'جدة', categoryId: allCats.find(c=>c.slug==='animals')!.id, userId: adminUser.id, images: JSON.stringify([img('1555169062-013468b47731')]), status: 'active', views: 45, phone: '0557654321', whatsapp: '966557654321' },
    ...
    { title: 'ساعة رولكس أصلية', description: 'ساعة رولكس أصلية موديل 2022 بحالة ممتازة مع الضمان', price: 35000, currency: 'ريال', city: 'جدة', categoryId: allCats.find(c=>c.slug==='fashion')!.id, userId: adminUser.id, images: JSON.stringify([img('1524805444758-089113d48a6d')]), status: 'active', isFeatured: true, views: 289, condition: 'مستعمل', phone: '0557654321', whatsapp: '966557654321' },
